...
if not csv_data.empty:
    csv_data['Value_num'] = pd.to_numeric(csv_data['Times Defeated'], errors='coerce')
    csv_data = csv_data.dropna(subset=['Value_num'])  # remove any NaN rows
    if not csv_data.empty:
        data_for_chart = csv_data.groupby('Villain')['Value_num'].sum()
        st.bar_chart(data_for_chart)
        st.write("This represents how many times Finn had to scramble each monster in the week.")
    else:
        st.info("No numeric data found in 'Times Defeated'.")
else:
    st.info("No data.")
...
if not csv_data.empty:
    ...
    if not csv_data.empty:
        ...
        if filtered_df.empty:
            ...
            group = filtered_df.groupby('Villain')['Value_num'].sum()
            st.bar_chart(group)
            st.write(
